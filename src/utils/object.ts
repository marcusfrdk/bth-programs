...
export const groupSemesters = (arr: ICourse[]): Record<string, ICourse[]> => {
	...
	const index = Array.from(new Set(arr.map((course) => {
	...
	for(let i=0;i<arr.length;i++){
		...
		if(nextWeek - currentWeek < 5) nextSemester = index[index.indexOf(course.start) + 2] || course.start;
		if (weeks > 10 && typeof nextSemester !== 'undefined') grouped[nextSemester].push(structuredClone(course));
