<script lang="ts">
	...
	$: if (program) {
		courses = groupSemesters(
			[...program.required_courses, ...program.optional_courses]
				.filter((f) => f?.start)
	...
	<ul>
		{#if Object.keys(courses).length > 0}
			{#each Object.entries(courses) as semester, i}
				...
				<li class:current={isCurrentSemester(semester)} class="semester">
					{semester[0].split(' ')[0]}<small>{getSemester(semester[0])}</small>
				</li>
